...
redirect_uri = 'https://d2o4tw11egl.cloudfront.net/asana.html'
    ...
    try:
        if event['httpMethod'] == 'OPTIONS':
            ...
            if not check_user_exists(username):
                ...
                # Add details and device_id to tokens
                tokens.update({
                    'details': 'none',
                    'device_id': 'none',
                    'height': 'none'
                ...
                # If the user exists, fetch their details from DynamoDB
                response = dynamodb.get_item(
                    TableName=table_name,
                    Key={'username': {'S': username}}
                )
                item = response.get('Item', {})
                details = item.get('details', {}).get('S', 'none')
                device_id = item.get('device_id', {}).get('S', 'none')
                height = item.get('height', {}).get('S', 'none')
                
                # Add fetched details and device_id to tokens
                tokens.update({
                    'details': details,
                    'device_id': device_id,
                    'height': height
